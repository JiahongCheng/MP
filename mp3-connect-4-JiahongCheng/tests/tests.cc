...
TEST_CASE("Drop2") {
  ...
  REQUIRE(b.board[0][6] == DiskType::kPlayer1);
  REQUIRE(b.board[1][6] == DiskType::kPlayer1);
}

TEST_CASE("OutOfBound") {
  Board b;
  InitBoard(b);

  REQUIRE_THROWS_AS(DropDiskToBoard(b,DiskType::kPlayer1, 7), std::runtime_error);
...
TEST_CASE("filled") {
  ...
  REQUIRE_THROWS_AS(DropDiskToBoard(b,DiskType::kPlayer1, 6), std::runtime_error);
}
